> **HISTORICAL (moved 2026-08-18 from `services/api/src/modules/chat/`).** This is
> the worker brief the chat module was built from. It is a record of *why* the
> module looks the way it does — the two seq scopes, the SSRF guard rules, the
> forward-only cursor invariant — and nothing more. **Do not build from it.** It
> is written in the imperative ("you are implementing", "implement ONLY the phase
...
> - **§6 `FREE_ATTACHMENT_MAX_MB` / `PREMIUM_ATTACHMENT_MAX_MB` and the
>   `attachmentMaxMb(store, userId)` lookup** — replaced by one flat
>   `ATTACHMENT_MAX_MB = 200` in `src/modules/chat/attachments.ts`, the same cap
