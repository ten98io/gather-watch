/**
 * Central error mapping: everything a route throws is translated to the
 * contracts ApiError shape here, so routes/services only ever throw AppError
 * (expected failures) or let unexpected errors fall through to a safe 500.
 */
import type { FastifyInstance } from 'fastify';
import { ZodError } from 'zod';
import { AppError, ERROR_STATUS } from '../lib/errors';

/** First zod issue rendered as "path.to.field: message" (bare message when
 *  the issue is on the root value). */
...
/**
 * AppError → its contracts payload at the mapped status; ZodError → 400
 * VALIDATION; rate-limit and JSON body-parse rejections → RATE_LIMITED /
 * VALIDATION; anything else is logged and answered with an opaque 500.
 */
