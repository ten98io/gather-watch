 ...
 * the API origin is inlined by tsup's `define` from GATHER_API_URL:
 *
 *   GATHER_API_URL=https://api.gather.watch pnpm --filter ./apps/extension build
 *
...
declare const __GATHER_API_URL__: string | undefined;
/**
 ...
 *
 *   GATHER_WEB_ORIGINS=https://gather.watch,https://www.gather.watch pnpm … build
 *
 ...
 */
declare const __GATHER_WEB_ORIGINS__: string | undefined;
...
export const API_URL =
  typeof __GATHER_API_URL__ === 'string' && __GATHER_API_URL__.length > 0
    ? __GATHER_API_URL__.replace(/\/+$/, '')
    : DEFAULT_API_URL;
 ...
 * forgotten or hijacked `*.gather.watch` host cannot reach the extension.
 */
...
const configuredWebOrigins = parseWebOrigins(
  typeof __GATHER_WEB_ORIGINS__ === 'string' ? __GATHER_WEB_ORIGINS__ : undefined,
);
