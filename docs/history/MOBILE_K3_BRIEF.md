> **HISTORICAL (moved 2026-08-18 from `apps/mobile/`).** This is the worker
> brief the Expo app was built from. It is a record of *why* the app looks the
> way it does — the logic-only test policy, the metro workspace config, the
> deliberately stubbed call surface — and nothing more. **Do not build from
> it.** It is written in the imperative ("you are creating", "OWN (create)")
> because it was an agent prompt; sitting in a live app directory it read as
> standing orders, which is why it moved.
>
> Four things in it are dead:
>
> - **The header and §READ FIRST cite `/Users/mg/Desktop/playin/BUILD_PROMPT.md`
>   as BINDING.** Nothing is at that path. The spec is
>   `docs/history/BUILD_PROMPT.md` and is itself marked do-not-build-from: its
>   plan is superseded (the extension is the primary playback driver now).
> - **"You are creating the Expo app from scratch — `apps/mobile/` does not
>   exist."** It exists and ships. Every file in the OWN tree is written; that
>   section is an inventory of what was built, not a task list.
> - **§ENVIRONMENT HARD RULES** — the sandbox rules, the `/tmp/gates-mobile`
>   rsync mirror, "NEVER touch anything outside `apps/mobile/`". These described
>   one agent's sandbox on one day. Gates run in the repo
>   (`pnpm --filter ./apps/mobile test`), and file scope is set per session.
> - **`CallBar.tsx` "join/leave call scaffold"** still describes the boundary
>   honestly, but its integration note names the ReplayKit stub as something
>   *`BUILD_PROMPT` marks*; treat `apps/mobile/src/components/CallBar.tsx`'s own
>   header as the live statement of that boundary.
>
> Everything else still describes live behaviour — including the RN-rendering
> ban in `tests/`, which is why mobile components have no render tests to this
> day. Current docs: `README.md`, `HANDOFF.md`, `docs/EXTENSION_FIRST.md`.
